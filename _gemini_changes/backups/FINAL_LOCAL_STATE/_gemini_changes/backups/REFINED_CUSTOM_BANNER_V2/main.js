    ...
    (function(){
        ...
        function toggleChip(btn){
            ...
            if(!key && typeof klaro !== 'undefined') {
                ...
                if(!manager.consents || !manager.consents.facebook) {
                    const banner = document.getElementById('variad-cookie-banner');
                    if(banner) {
                        // Text temporär ändern für den "Lock"-Effekt
                        const p = banner.querySelector('.text-side p');
                        const originalText = p.textContent;
                        p.textContent = "Akzeptiere erst die Cookies.";
                        banner.classList.add('is-visible');
                        
                        // Wenn der User im Banner interagiert, wird er eh geschlossen.
                        // Wir müssen nur sicherstellen, dass der Text beim nächsten Mal wieder stimmt.
    ...
    (function() {
        window.addEventListener('load', function() {
            setTimeout(function() {
                ...
                // Button: Akzeptieren
                document.getElementById('v-accept-btn').onclick = function() {
                    const consents = {};
                    manager.config.services.forEach(s => { consents[s.name] = true; });
                    manager.updateConsents(consents);
                    manager.saveAndApplyConsents();
                    banner.classList.remove('is-visible');
                };

                // Button: Ablehnen
                document.getElementById('v-decline-btn').onclick = function() {
                    manager.updateConsents({});
                    manager.saveAndApplyConsents();
                    banner.classList.remove('is-visible');
                };

                // Link: Einstellungen (Öffnet originales Klaro-Modal)
                document.getElementById('v-open-settings').onclick = function() {
                    banner.classList.remove('is-visible');
                    // Wir nutzen Klaro's internen Weg um das Modal zu zeigen
                    klaro.show(); 
                };
            }, 500);
        });
