        ...
        function toggleChip(btn){
            ...
            if(!key && typeof klaro !== 'undefined') {
                ...
                if(!manager.consents || !manager.consents.facebook) {
                    console.log('[Wizard] Blocked: Cookies missing');
                    const banner = document.getElementById('variad-cookie-banner');
                    if(banner) {
                        const p = banner.querySelector('.text-side p');
                        const originalText = "Wir nutzen Cookies für Performance & Marketing.";
                        p.textContent = "Akzeptiere erst die Cookies.";
                        banner.classList.add('is-visible');
                        
                        // Handler für den speziellen Lock-Zustand
                        const resetAndProcess = () => {
                            console.log('[Wizard] Resetting banner text');
                            p.textContent = originalText;
                        };
                        
                        // Wir hängen uns an den Klick, um den Text zurückzusetzen
                        document.getElementById('v-accept-btn').addEventListener('click', resetAndProcess, {once:true});
                        document.getElementById('v-decline-btn').addEventListener('click', resetAndProcess, {once:true});
                    }
                    return; 
                }
            }
